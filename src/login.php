...
if (isset($_POST['form_sent']) && $action == 'in')
{
	...
	// --- Brute-force protection: per-IP + per-username attempt throttling ---
	$lockout_max  = 5;          // allowed attempts
	$lockout_window = 900;      // seconds (15 min) the counter is tracked
	$lockout_block  = 1800;     // seconds (30 min) locked out after exceeding

	$client_ip = get_remote_address();
	$attempt_sql = 'SELECT COUNT(*) AS c, MAX(attempted) AS last FROM '.$db->prefix.'login_attempts WHERE (ip=\''.$db->escape($client_ip).'\' OR username=\''.$db->escape($form_username).'\') AND attempted > '.(time() - $lockout_window);
	$attempt_res = $db->query($attempt_sql) or error('Unable to check login attempts', __FILE__, __LINE__, $db->error());
	$attempt_row = $db->fetch_assoc($attempt_res);
	if ($attempt_row['c'] >= $lockout_max)
	{
		$remaining = $lockout_block - (time() - intval($attempt_row['last']));
		message(sprintf('Too many failed login attempts. Please wait %d minute(s) before trying again.', max(1, ceil($remaining / 60))));
	}
	...
	{
		// Strong bcrypt/argon2 + legacy sha1/md5 verification with auto-upgrade.
		$verified = pun_verify_password($form_password, $cur_user['password']);

		if ($verified !== false)
		{
			$authorized = true;

			// Upgrade legacy hashes (or stale cost) to bcrypt on successful login.
			if ($verified === 2)
			{
				$new_hash = pun_hash_password($form_password);
				$db->query('UPDATE '.$db->prefix.'users SET password=\''.$db->escape($new_hash).'\' WHERE id='.$cur_user['id']) or error('Unable to update user password', __FILE__, __LINE__, $db->error());
			}
		}
	}
	...
	if (!$authorized)
	{
		...
		$db->query('INSERT INTO '.$db->prefix.'login_attempts (ip, username, attempted) VALUES(\''.$db->escape($client_ip).'\', \''.$db->escape($form_username).'\', '.time().')') or error('Unable to log login attempt', __FILE__, __LINE__, $db->error());
		...
		if (empty($errors))
		{
			...
			if ($db->num_rows($result))
			{
				...
				while ($cur_hit = $db->fetch_assoc($result))
				{
					...
					$db->query('UPDATE '.$db->prefix.'users SET activate_string=\''.pun_hash_password($new_password).'\', activate_key=\''.$new_password_key.'\', last_email_sent = '.time().' WHERE id='.$cur_hit['id']) or error('Unable to update activation data', __FILE__, __LINE__, $db->error());
