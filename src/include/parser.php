...
function handle_url_tag($url, $link = '', $bbcode = false)
{
	...
	// Only allow safe link schemes. Anything else is neutralised by forcing
	// http:// in front of it (so e.g. "javascript:..." becomes a harmless URL
	// to a non-resolvable host rather than an executable pseudo-protocol).
	$safe_scheme = preg_match('#^(https?|ftp|irc)://#i', $url) || strpos($url, 'www.') === 0 || strpos($url, 'ftp.') === 0 || strpos($url, '/') === 0;
	...
	// Escape the generated href so a crafted value can never break out of the attribute.
	$full_url = pun_htmlspecialchars($full_url);
...
function handle_img_tag($url, $is_signature = false, $alt = null)
{
	...
	$img_tag = '<a href="'.$safe_url.'" rel="nofollow">&lt;'.$lang_common['Image link'].' - '.$alt.'&gt;</a>';

	if ($is_signature && $pun_user['show_img_sig'] != '0')
		$img_tag = '<img class="sigimage" src="'.$safe_url.'" alt="'.pun_htmlspecialchars($alt).'" />';
	else if (!$is_signature && $pun_user['show_img'] != '0')
		$img_tag = '<span class="postimg"><img src="'.$safe_url.'" alt="'.pun_htmlspecialchars($alt).'" /></span>';
